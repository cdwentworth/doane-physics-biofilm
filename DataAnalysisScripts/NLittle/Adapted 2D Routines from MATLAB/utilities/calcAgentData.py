...
def result = calcAgentData(adata,theval):
    ...
    if strcmp(theval,'number')
    ...
    else if strcmp(theval,'simpsonindex') || strcmp(theval,'simpsonindex_noeps')
            nEach = zeros(nspec,1);
        ...
        for ispec=1:nspec
            ...
            #######################################################
            '''Above note is copied from original script'''

            result = result + pi*sum(rads.*rads);

    else if strcmp(theval,'center') || strcmp(theval,'stddev') || strcmp(theval,'spread')
        ...
        else if strcmp(theval,'stddev')
            result = [std(xc-mean(xc)),std(yc-ycmean),std(zc-mean(zc))];
        else if strcmp(theval,'spread')
    ...
    else if strcmp(theval,'center_nonper')
        [xc,yc,zc] = getCoords(adata);
        xc = mean(xc);
        yc = mean(yc);
        zc = mean(zc);
    ...
    else if strcmp(theval,'stddev_nonper')
        [xc,yc,zc] = getCoords(adata);
        xc = std(xc);
        yc = std(yc);
        zc = std(zc);
    ...
    else if strcmp(theval,'spread_nonper')
        [xc,yc,zc] = getCoords(adata);
        xc = max(xc) - min(xc);
        yc = max(yc) - min(yc);
        zc = max(zc) - min(zc);
    ...
    else if strcmp(theval,'averagesize')
...
def [xc,yc,zc] = getCoords(specdata):
