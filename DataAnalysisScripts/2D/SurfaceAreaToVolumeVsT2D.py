...
Script to calculat surface area to volume ratio of the two-dimensional film 
...
def findAvg(xlist,ylist,numCells):
...
for f in recordNames:
    xlist,ylist,zlist,resolution, nJ,numCells=loci(name,f)
    AvgSV = findAvg(xlist,ylist,numCells)
    AvgSVArray.append(AvgSV)
    m = re.search('\((.+?)\)',f)
    t = int(m.group(1))
    timeArray.append(t)
for f in recordNames:
...
outFile.write('Surface to Volume - Time\n')
outFile.write('t [hour]   SVR \n')
numValues = len(timeArray)
for t in range(numValues):
    outFile.write(' %6.1f \t%10.4f\n' % (timeArray[t],AvgSVArray[t]) )
outFile.close()
plt.plot(timeArray, AvgSVArray)
plt.xlabel('Time [h]')
plt.ylabel(' SVR')
plt.grid(True)
plt.draw()
